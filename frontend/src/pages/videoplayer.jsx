...
export const VideoIndv = ({ darkMode }) => {
    ...
    return (
        <div className={`py-2 ${darkMode ? "bg-neutral-800 text-white" : "bg-white"}`}>
            {/* Video player */}
            <div className="h-[70%]">
                <video
            ...
            <div className={` ${desOpen ? "opacity-0" : ""} flex w-full justify-between md:w-[70%] mt-4 h-[40px] items-center gap-5`}>
                <h2 className={`text-xl ml-3 w-[88%] truncate font-bold ${darkMode ? "text-white" : "text-black"}`}>{video.title}</h2>
                <ChevronDown
            ...
            {desOpen && (
                <div
                    className={`fixed inset-0 ${!darkMode ? "bg-neutral-900 text-white" : "bg-white"} bg-opacity-50 z-40
                        `}
                    onClick={() => setDesOpen(false)}
                >
                    <div
                        className={`absolute bottom-0 ${darkMode ? "bg-neutral-900 text-black" : "bg-white"} w-[100%] left-0 right-0 md:top-[30%] md:h-auto h-[60%] md:mx-auto md:w-[80%] pt-4 pl-5 ${darkMode ? " bg-neutral-950 text-white" : "bg-white"}   bg-white rounded-t-lg p-4 z-50
