...
const ChangeAvatar = ({ darkMode }) => {
  ...
  return (
    <div className={`min-h-screen p-6 ${darkMode ? "bg-black text-white" : "bg-gray-100 text-black"}`}>
      <div className={`max-w-xl md:max-w-2xl mt-[20%] md:mt-0 mx-auto p-6 rounded-2xl border ${darkMode ? "bg-zinc-900 border-zinc-700" : "bg-white border-gray-300"}`}>
        <h1 className="text-2xl md:text-3xl font-bold mb-6">
          Change Avatar
        </h1>

        <form className="w-[80%] md:w-full mt-3" onSubmit={handleSubmit}>
          <label className="m-auto p-2 md:p-4 rounded-lg border cursor-pointer 
  ${darkMode ? 'bg-zinc-800 text-[13px] md:text-[24px] border-zinc-700 text-black' : 'bg-white border-gray-300'}">
            Upload Image
            <input
              type="file"
              accept="image/*"
